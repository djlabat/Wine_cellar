...
export class WineListComponent implements OnInit {
  wineList:Wine[] = [] // ovo polje ce se u "wine-list.component.html" povezati sa inputom  iz "table.component.ts"
  
  constructor() {
    for (let wineObjIzWine of WINES) {
      this.wineList.push(new Wine(wineObjIzWine))// ← u "wineList" se pravi lista objekata iz "WINES"
      /* ↑ ako stavim ovako "this.wineList.push(wineObjIzWine)"
       onda "wineList" biva popunjen sa objektima iz konstante WINES,
       ali nije proslo kroz validaciju koja postoji u modelu Wine.
       Znaci u ovom slucaju rezultat je isti, zato sto su svi objekti validni,
       ali da je postoja neki invalidni objekat u "WINES",
       onda bi taj objekat usao u listu, takav kakav jeste, invalidan, sa svojim parametrima i vrednostima
       
       a kada bi prosao kroz validaciju onda bi usao u listu, ali bi imao parametre _id, name...,
        i svi bi bili sa null vrednostima*/
    }
    
   }

  ngOnInit(): void {
    // ovde mogu da stavim sta ce da se desi kada se ova komponenta ucita. npr console.log(this.wineList)
